...
def split_sum_and_answer(x_str):
    ...
    return list(np.array(x_str)[:,0]), list(np.array(x_str)[:,1])
...
def get_addition_dataset(N, max_operand_digits=2, seed=1, val_prop=0.1):
    ...
    addition_expressions = make_addition_expressions(N,max_operand_digits)
    ...
    x_train_test, y_train_test, x_val_test, y_val_test = make_train_val_split(x_int_test.copy(), y_int_test.copy())
